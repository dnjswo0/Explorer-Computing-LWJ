...
def section_2_2():
    ...
    tab1, tab2, tab3 = st.tabs(["1) 모비율 검정", "2) 회귀분석", "3) 모평균 검정"])
...
def section_3_1():
    st.header("3.1 시사점")
    st.subheader("🔎 본 탐구의 의의")
    st.write("""
    본 탐구에서는 '밴드 붐이 온다'라는 유행어에 착안하여 실제로 '밴드 붐'이 왔는지 알아보고자 하였습니다.
    ...
    이에 멜론 차트 음원 데이터, 펜타포트 관람객 수 등의 자료를 활용하여
    두 집단의 모비율, 회귀분석, 두 집단의 모평균 검정 등을 수행하여 가설을 설정하고 결론을 내렸습니다.

    멜론 차트의 음원 데이터를 통해 모비율 검정, 모평균 검정 등을 수행한 결과 유의미하게 록/메탈 곡 수가 차트에서 증가하였고,
    이는 밴드 붐이 왔다는 주장의 근거로 뒷받침될 수 있습니다.
    그러나 연도를 설명변수, 곡 수를 반응변수로 설정하고 회귀분석을 수행한 결과 직선관계는 유의미하지 않았고,
    이는 변수 간 비선형적 관계 때문이라는 것을 산점도를 통해 알 수 있었습니다.


    본 탐구는 이처럼 하위 문화를 둘러싼 담론을 경험적 자료로 검토함으로써, 
    문화 현상이 단순한 체감이나 인상이 아니라 사회적으로 구성된 과정임을 확인하고자 했다는 점에서 의의를 가집니다.
    """)
